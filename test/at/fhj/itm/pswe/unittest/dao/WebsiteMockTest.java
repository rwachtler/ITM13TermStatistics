...
public class WebsiteMockTest {
	...
	public void testfindAllWebsites() {
		...
		EntityManager mockEm = createMock(EntityManager.class);
		expect(mockEm.createQuery("SELECT DISTINCT w FROM Website w ORDER BY w.id", Website.class))
				.andReturn(mockedQuery);
		replay(mockEm);
	...
	public void testarticleTimelineofOneWebsite() {
		...
		// EM Mock
		EntityManager mockEm = createMock(EntityManager.class);
		expect(mockEm.createQuery("SELECT COUNT(DISTINCT c.article.url), c.logDate, c.article.id "
	...
	public void testfindWordsOfSite() {
		...
		// EM Mock
		EntityManager mockEm = createMock(EntityManager.class);
		expect(mockEm.createQuery("SELECT c.word.text, sum(c.amount) "
				+ "FROM Container c WHERE c.website.id=:id AND c.word.active = TRUE "
				+ "GROUP BY c.word ORDER BY sum(c.amount) DESC")).andReturn(mockedQuery);
		replay(mockEm);
	...
	public void testtimeLine4WordAndSite() {
		...
		expect(mockEm.createQuery("SELECT co.logDate, SUM(co.amount) " + "FROM Container co "
				...
				+ "AND co.word.text = :word " + "GROUP BY co.word.text, co.logDate")).andReturn(mockedQuery);
		replay(mockEm);
	...
	public void testfindAllArticlesOfOneWebsite() {
		...
		// EM Mock
		EntityManager mockEm = createMock(EntityManager.class);
		expect(mockEm.createQuery("SELECT DISTINCT c.article.id, c.article.url "
				+ "FROM Container c WHERE c.website.id=:id ORDER BY c.article.id")).andReturn(mockedQuery);
		replay(mockEm);
