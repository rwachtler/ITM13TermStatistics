...
public class ArticleMockTest {
	...
	public void TestfindWordsOfArticle() {
		...
		expect(mockEm.createQuery(
				"SELECT SUM(c.amount), c.logDate " + "FROM Container c WHERE c.article.id=:id AND c.word.active = TRUE "
						+ "GROUP BY c.article.id, c.logDate ORDER BY SUM(c.amount) DESC")).andReturn(mockedQuery);
		replay(mockEm);
	...
	public void TestgetAVGAnalyzeDurationofArticle() {
		...
		EntityManager mockEm = createMock(EntityManager.class);
		expect(mockEm.createQuery("SELECT SUM(ast.analyzeDuration)/COUNT(ast.analyzeDuration) "
				+ "FROM ArticleStat ast WHERE ast.article.id=:id ORDER BY ast.article.url")).andReturn(mockedQuery);
		replay(mockEm);
		ArticleDao aDAO = new ArticleDao();
		aDAO.setEntityManager(mockEm);
		// Test
	...
	public void testupdateArticle() {
		...
		EntityManager mockEm = createMock(EntityManager.class);
		expect(mockEm.createQuery("SELECT ar.id, ar.url FROM Article ar WHERE ar.url = :url")).andReturn(mockedQuery);
