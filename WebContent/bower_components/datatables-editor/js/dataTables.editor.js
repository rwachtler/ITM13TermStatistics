...
$.extend( Editor.DateTime.prototype, {
	...
	val: function ( set, write ) {
		...
		if ( set instanceof Date ) {
			this.s.d = new Date( Date.UTC(
				set.getFullYear(), set.getMonth(), set.getDate(),
				set.getHours(), set.getMinutes(), set.getSeconds()
			) );
		}
	...
	_htmlMonth: function ( year, month ) {
		var now    = new Date(),
			...
			before = new Date( year, month, 1 ).getUTCDay(),
			data   = [],
			row    = [];
